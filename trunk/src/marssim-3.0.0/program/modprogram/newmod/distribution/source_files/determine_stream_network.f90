    ...
    SUBROUTINE FIND_STREAM_NETWORK()
        ...
        USE ERODE_GLOBALS
        IMPLICIT NONE
        INTEGER(4),ALLOCATABLE, DIMENSION(:) :: ORDER,MAGNITUDE,ILOC,JLOC
        ...
        REAL (8) :: ATOP,ABOT
        REAL (8) :: XDIST,INDSUM,INDSSQ,XSTART,YSTART,XSUM,XEND,YEND
        REAL (8) :: XXXX,XEXTSUM,XEXTSSQ,XMAGSUM,XMAGSSQ,XINTSUM,XINTSSQ
        REAL (8) :: NTS,LDENSUM,LDENSSQ,CORREL,TOTAL,SUMX,SUMY,SUMXY
        REAL (8) :: SUMXX,SUMYY,KAPPA,FORMFAC,TCHANLEN,ARRATSUM,ARRATSSQ
        REAL (8) :: RATARSUM,RATARSSQ,LINKSLP,LDENSLP,SLPSUM,SLPSSQ
        REAL (8) :: ETOP,GTOP,EBOT,GBOT,LRATIO,LTOT,TEMP,TEMP1,ARRAT,XINTMAG
        REAL (8) :: LINKDEN,ARRATSLP
        !      *********************************************************************
        ...
        !             iloc - (i) the i-location of the matrix location
        !             jloc - (i) the j-location of the matrix location
        ...
        !      *********************************************************************
        ALLOCATE(ORDER(LMMX),MAGNITUDE(LMMX),ILOC(LMMX),JLOC(LMMX),STAT=IERROR)
        ...
        !           point to our vector, recording the i- and j- locations (iloc,jloc)
        ...
        !         same value of iloc and jloc.  The first 7 lines of the do loop are
        ...
        !      *********************************************************************
        DO  K=1,KSEGS
            LOCAL_DIRECTION = FLOW_DIRECTION(ILOC(K),JLOC(K))
            IF (LOCAL_DIRECTION < 0) CYCLE 
            J_NEW=JLOC(K)+DOWNSTREAM(LOCAL_DIRECTION,2)
            I_NEW=ILOC(K)+DOWNSTREAM(LOCAL_DIRECTION,1)
            ...
            ENDIF
            DO  L=1,KSEGS
                IF ((ILOC(L) == I_NEW).AND.(JLOC(L) == J_NEW)) THEN
                    TOSEGMENT(K)=L
                    EXIT
                ENDIF
            ENDDO
        ...
        DO  K=1,KSEGS
            IF (SOURCE(K)) THEN
                ...
                L115: DO
                    ...
                    IF (FLOW_DIRECTION(ILOC(L),JLOC(L)) > 5) THEN
                        XSUM=XSUM+SQRTOFTWO
                    ELSE
                        XSUM=XSUM+1.0
                    ENDIF
        ...
        L131: DO  K=1,KSEGS
            IF (SOURCE(K)) THEN
                L=K
                ATOP = DRAINAGE_AREA(ILOC(K),JLOC(K))
                ETOP = ELEVATION(ILOC(K),JLOC(K))
                GTOP = D8_GRADIENT(ILOC(K),JLOC(K))
                XSUM=0.0
                L132: DO
                    IF (FLOW_DIRECTION(ILOC(L),JLOC(L)) > 5) THEN
                        XSUM=XSUM+SQRTOFTWO
                    ELSE
                        XSUM=XSUM+1.0
                    ENDIF
                    ABOT = DRAINAGE_AREA(ILOC(L),JLOC(L))
                    EBOT = ELEVATION(ILOC(L),JLOC(L))
                    GBOT = D8_GRADIENT(ILOC(L),JLOC(L))
                    L=TOSEGMENT(L)
                    IF (L == 0) EXIT L132
        ...
        WRITE (OUTSUMMARY,908) TEMP,TEMP1,LTOT
        DEALLOCATE(ORDER,MAGNITUDE,ILOC,JLOC,TOSEGMENT)
