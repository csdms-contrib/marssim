    ...
    SUBROUTINE DO_LAVA_FLOWS(NNNN)
        ...
        REAL (8) :: MINEL,PROBCOMP
        REAL (4) :: NLACTIVE1,NLACTIVE2
        REAL (8) :: TVOLUME,DTEMP, RRAND
        EXTERNAL RRAND
        !      *******************************************************************
        ...
        L100: DO LAVAITER=1,SOURCE_CHANGE_INTERVAL
            ...
            IF (PROBCOMP > ERUPTION_STOP_PROBABILITY) THEN
                ...
                IF ((KKK > 0).AND.(NUMBER_OF_ACTIVE_LAVA_CELLS < RMMX))THEN
                    ...
                    ELEVATION(INX,JNX)=DMIN1((ELEVATION(INX,JNX)+LAVA_FLOW_THICKNESS) &
        ...
        998   IF (TACTSAMP > 1.0) THEN
            TLACTIVESQ=DSQRT((TLACTIVESQ-TLACTIVESUM*TLACTIVESUM/TACTSAMP)  &
            /(TACTSAMP-1.0))
        ...
        IF (TVOLUMESAMP > 1.0) THEN
            TVOLUMESQ=DSQRT((TVOLUMESQ-TVOLUMESUM*TVOLUMESUM/TVOLUMESAMP) &
            /(TVOLUMESAMP-1.0))
    ...
    SUBROUTINE FIND_LAVA_START_PLACE(IST,JST,LLL,I_NEW,J_NEW)
        ...
        USE LAVA_GLOBALS
        IMPLICIT NONE
        REAL (8) :: SPROB(9),EMIN,LOCALGRAD,PROBSUM,PROBCOMPARE, RRAND
        REAL (4) :: RANDTEMP
        EXTERNAL RRAND
        ...
        L110: DO L=2,9
            ...
            IF (LOCALGRAD > 0.0) THEN
                ...
                !          maximum probability
                !      *******************************************************************
                SPROB(L)=(1.0-DEXP(-LAVA_GRADIENT_WEIGHT*LOCALGRAD))
            ELSE
                !      *******************************************************************
    ...
    SUBROUTINE FIND_NEXT_LAVA_SITE(LLL,KKK,INX,JNX)
        ...
        REAL (8) :: LOCALGRAD,PROBSUM,PROBCOMPARE, RRAND
        REAL (4) :: RANDTEMP
        EXTERNAL RRAND
        !      *******************************************************************
                ...
                IF (LAVA_FLOW_PROBABILITY(L,K) /= 0.0) THEN
                    ...
                    THEN
                        IF (L > 5) THEN
                            LOCALGRAD=LOCALGRAD*0.7071068
                        ENDIF
                        LAVA_FLOW_PROBABILITY(L,K)=(1.0-DEXP(-LAVA_GRADIENT_WEIGHT*LOCALGRAD))* &
                        DEXP(-LAVA_DURATION_WEIGHT*LAVA_ELAPSED_TIME(K))
                    ELSE
