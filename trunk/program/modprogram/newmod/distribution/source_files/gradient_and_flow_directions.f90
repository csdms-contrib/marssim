    ...
    SUBROUTINE GRADIENT_AND_FLOW_DIRECTION()
        ...
        IMPLICIT NONE
        REAL (8) :: PROVGRAD,FF,GRADIENT_RATIO,GRADOLD,GRADAVG,NGRAD, RRAND
                ...
                DO  J=1,MYY
                    IF (FLOW_DIRECTION(I,J) > 5) THEN
                        ...
                        IF (IS_Y_PERIODIC) THEN
                            ...
                            IF (JPROV < 1) JPROV=MY
                        ENDIF
                        D8_GRADIENT(I,J)=(ELEVATION(I,J)-ELEVATION(IPROV,JPROV))*0.7071068
                    ENDIF
                ENDDO
            ENDDO
        ENDIF
                    ...
                    IF (IS_SEDIMENT_COVERED(I,J).AND.(FLOW_DIRECTION(I,J) > 1)) THEN
                        ...
                        IF (IIIDO > 1) THEN
                            IF (FLOW_DIRECTION(I,J) /= IIIDO) THEN
                                GRADOLD=ELEVATION(I,J)-ELEVATION(I+DOWNSTREAM(IIIDO,1),J+DOWNSTREAM(IIIDO,2))
                                IF (GRADOLD > 0.0) THEN
