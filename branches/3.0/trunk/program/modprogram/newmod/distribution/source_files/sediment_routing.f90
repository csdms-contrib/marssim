    ...
    !        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    !        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    SUBROUTINE FIND_DOWNSTREAM_LOCATION(II,JI,IIO,JJO,LOCAL_DIRECTION)
        USE ERODE_GLOBALS
        ...
        INTEGER :: IDEP
        REAL (8) ::  FF,EL1,EL2,MINGRAD
        REAL (8) ::  XGRAD(9)
        LOGICAL :: KOK(9),ISCIRCLE
        ...
        IF ((II == IIO).AND.(JI == JJO)) THEN
            ...
            ELSE
                ISUB=0
            ENDIF
            WRITE(*,707) II,JI,FLOW_DIRECTION(IIO,JJO),NNN,ISTART,JSTART, &
        ...
        IF (ICOUNT > IDMAX) THEN
            ...
            WRITE(OUTHIST,200) ISTART,JSTART,IEND,JEND,INOW,JNOW,II,JI,IIO,&
            ...
            WRITE(OUTHIST,210) FLOW_DIRECTION(IEND,JEND),FLOW_DIRECTION(INOW,JNOW),FLOW_DIRECTION(IIO,JJO),NNN
        ...
        IF ((IIO > MX).OR.(IIO < 1).OR.(JJO < 1).OR.(JJO > MY)) THEN
            ...
            WRITE(OUTHIST,510) II,JI,IIO,JJO,LOCAL_DIRECTION,FLOW_DIRECTION(II,JI),ISTART,JSTART,&
            ...
            WRITE(OUTHIST,511) NNN,I1,I2,INCRX,INCRY,ICOUNT
        ...
        IF ((II == IIO).AND.(JI == JJO)) THEN
            ...
            IEND,JEND,INOW,JNOW,DONE_ONCE(IIO,JJO)
            WRITE(OUTHIST,708) II,JI,IIO,JJO,LOCAL_DIRECTION,FLOW_DIRECTION(II,JI),ISTART,JSTART,&
            IEND,JEND,INOW,JNOW,IDEPR,DEFAULT_SEDIMENT_YIELD,SEDIMENT_CARRYOVER, &
            DEFAULT_DRAINAGE_AREA,IMUST,IROUND,IBACK,IDONE,ICIRCLE,INDEPRESS,&
            IQIN,IQNEXT
            WRITE(OUTHIST,511) NNN,I1,I2,INCRX,INCRY,ICOUNT
    ...
    SUBROUTINE WRITE_DEBUG_DATA(II,JJ)
        ...
        DO J=JLOW,JHIGH
            WRITE(OUTHIST,200) (FLOW_DIRECTION(I,J),I=ILOW,IHIGH)
            200 FORMAT(5(I2,' '))
        ENDDO
        WRITE(OUTHIST,250)
        ...
        DO J=JLOW,JHIGH
            ...
            DO I=ILOW,IHIGH
               ...
               IX=IX+1
            ENDDO
            WRITE(OUTHIST,200) (ISUBM(I),I=1,5)
        ENDDO
        WRITE(OUTHIST,300)
        300 FORMAT(/,' ELEVATIONS')
        DO J=JLOW,JHIGH
            WRITE(OUTHIST,350) (ELEVATION(I,J), I=ILOW,IHIGH)
            350 FORMAT(5(G12.5,' '))
        ENDDO
        WRITE(OUTHIST,400)
        400 FORMAT(/,' GRADIENTS')
        DO J=JLOW,JHIGH
            WRITE(OUTHIST,350) (D8_GRADIENT(I,J),I=ILOW,IHIGH)
        ENDDO    
        RETURN
    END
    !        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    SUBROUTINE SEDIMENT_TRANSPORT_FLUX(BEDLOAD_FLUX,TRANSPORT_STAGE,GRADIENT,A1TERM,A2TERM)
        ...
        !        *********************************************************************
        REAL (8), INTENT(OUT):: BEDLOAD_FLUX,TRANSPORT_STAGE
        REAL (8), INTENT(IN) :: GRADIENT,A1TERM,A2TERM
        REAL (8):: XARGUMENT
    ...
    SUBROUTINE EQUILIBRIUM_SEDIMENT_GRADIENT(BEDLOAD_FLUX,GRADIENT,DISCHARGE_VALUE)
        ...
        !        *********************************************************************
        REAL (8), INTENT(IN):: BEDLOAD_FLUX,DISCHARGE_VALUE
        REAL (8), INTENT(OUT) :: GRADIENT
        REAL (8) :: TERM1,DISCHARGE_TERM
        DISCHARGE_TERM=MAX(DISCHARGE_VALUE,0.1D00)*STEADY_DISCHARGE_FACTOR
    ...
    SUBROUTINE SEDIMENT_FLUX_DIVERGENCE()
        ...
        REAL (8) :: XQQQ(9),XQSUM,XQMAX,A1TERM,A2TERM,X_DISCHARGE,GRADIENT,BEDLOAD_FLUX, &
    ...
    SUBROUTINE ROUTE_SEDIMENT(I,J,DELT,BACKGRADE,SEDSOURCE,S1,S2)
        ...
        REAL (8):: CRITELCHANGE
        PARAMETER (CRITELCHANGE=10.0)
        REAL (8) ::  EGRADSUM,GDIFFSUM,  &
        ...
        LOGICAL :: FIXED,MUSTDOIT
        LOGICAL :: PRINTSTUFF,REVISIT
        INTEGER :: KLOWER
        INTEGER :: I999,I130,I119,I567,I120,I220,IXXX,JXX,IQX
        INTEGER :: I1220,I903
        REAL (8) :: TEMPAREA,ELX,GRADX,EGRADX,SEDX
        REAL (8) :: DELEL,DELEX,TEMPP,BEDLOAD_FLUX,ABSMAXCHG,LOCCHANGE
        !        *********************************************************************
        ...
        L999: DO
            ...
            FIXED=.TRUE.
            !        *********************************************************************
            !         is_bedrock_channel is false if the local site is alluvial channel, otherwise it is false
            ...
            ENDIF
            IF (COMPLETE_RUNOFF) THEN
                REFWATERLEVEL=DMAX1(OCEAN_ELEVATION,LAKE_SURFACE_ELEVATION(BASIN_NUMBER(II,JJ)))
            ELSE
            ...
            I_OLD=II
            J_OLD=JJ
            CALL FIND_DOWNSTREAM_LOCATION(I_OLD,J_OLD,I_NEW,J_NEW,LOCAL_DIRECTION)
            I_LOCATION(K)=I_NEW
            J_LOCATION(K)=J_NEW
            IF (COMPLETE_RUNOFF) THEN
                WATER_LEVEL(K)=DMAX1(OCEAN_ELEVATION,LAKE_SURFACE_ELEVATION(BASIN_NUMBER(I_NEW,J_NEW)))
            ELSE
            ...
            IF (DELEL > 0.0) THEN
                ...
                IF (DELEX < START_STEP_DISTANCE) THEN
                    SEDQMAX=DMAX1(SEDQMAX,MAXIMUM_SEDIMENT_YIELD(II,JJ))
                    DISCHARGEMAX=DMAX1(DISCHARGEMAX,MAXIMUM_DISCHARGE(II,JJ))
            ...
            L130: DO
                ...
                I_OLD=I_NEW
                J_OLD=J_NEW
                SEDQMAX=DMAX1(MAXIMUM_SEDIMENT_YIELD(I_OLD,J_OLD),SEDQMAX)
                BEDLOAD_FLUX=SEDQMAX
                DISCHARGEMAX=DMAX1(MAXIMUM_DISCHARGE(I_OLD,J_OLD),DISCHARGEMAX)
                ...
                !        *********************************************************************
                IF (COMPLETE_RUNOFF) THEN
                    WATER_LEVEL(K+1)=DMAX1(OCEAN_ELEVATION,LAKE_SURFACE_ELEVATION(BASIN_NUMBER(I_NEW,J_NEW)))
                ELSE
                ...
                ELSE
                    ...
                    SEDQMAX=DMAX1(SEDQMAX,MAXIMUM_SEDIMENT_YIELD(I_OLD,J_OLD))
                    BEDLOAD_FLUX=SEDQMAX
                    DISCHARGEMAX=DMAX1(MAXIMUM_DISCHARGE(I_OLD,J_OLD),DISCHARGEMAX)
                    IF (SEDQMAX > MAXIMUM_SEDIMENT_YIELD(II,JJ)) MAXIMUM_SEDIMENT_YIELD(II,JJ)=SEDQMAX
                    IF (DISCHARGEMAX > MAXIMUM_DISCHARGE(II,JJ)) MAXIMUM_DISCHARGE(II,JJ)=DISCHARGEMAX
                ...
                IF (SEDSOURCE) THEN
                    ...
                    IF (K > 1) THEN
                        ...
                        ENDDO
                    ENDIF
                    PROVISIONAL_END_GRADIENT=ACTUAL_GRADIENT(K)+(SEDIMENT_VOLUME/D2X+GDIFFSUM+ &
                ...
                ELSE
                    ...
                    !        *********************************************************************
                    PROVISIONAL_START_GRADIENT=ACTUAL_START_GRADIENT-(SEDIMENT_VOLUME/D2X+GDIFFSUM)/(K*START_STEP_DISTANCE)
            ...
            IF (.NOT.IS_SEDIMENT_COVERED(KI,KJ)) THEN
                ...
                ENDIF
            ENDIF
            VOLSTART=STARTING_ELEVATION(1)*D2X
            VOLEND=NEW_ELEVATION(1)*D2X
            IF (K > 1) THEN
                L445: DO  L=2,K
                    KI=I_LOCATION(L)
                    KJ=J_LOCATION(L)
                    ...
                    ENDIF
                    VOLSTART=VOLSTART+STARTING_ELEVATION(L)*D2X
                    VOLEND=VOLEND+NEW_ELEVATION(L)*D2X
                    IF (.NOT.IS_SEDIMENT_COVERED(KI,KJ)) THEN
                        IF (ELEVATION(KI,KJ) < SED_SURFACE_ELEVATION(KI,KJ)) THEN
                            CHANNEL_STATE_CHANGE(KI,KJ)=1
                            ICASE(5)=ICASE(5)+1
            ...
            DO L=1,K-1
                ...
                IF (TEMPP < 0.0) THEN
                    WRITE(*,3882) I_LOCATION(L),J_LOCATION(L),TEMPP,NEW_ELEVATION(L),NEW_ELEVATION(L+1),WATER_LEVEL(L)
            ...
            IF (ABSMAXCHG >= CRITELCHANGE) THEN
                ITYYP=1
                CALL  PRINT_SEDIMENT_DIAGNOSTICS(IMAX,JMAX,II,JJ,ITYYP,SEDSOURCE)
            ENDIF
            !        *********************************************************************
            ...
            IF (IS_SEDIMENT_COVERED(I_NEW,J_NEW).OR.(CHANNEL_STATE_CHANGE(I_NEW,J_NEW) == 1)) THEN
                IF (DABS(SEDREMAIN) > (0.01*SEDIMENT_VOLUME)) THEN
                    IF (BACKGRADE) THEN
            ...
            IF (BACKGRADE.AND.(SEDREMAIN > 0.0)) THEN
                ...
                L567: DO
                    ...
                    ELSE
                        ...
                        ENDIF
                    ENDIF
                    CALL FIND_DOWNSTREAM_LOCATION(I_NEW,J_NEW,IXXX,JXX,IQX)
                    ...
                    ENDIF
                    IF (COMPLETE_RUNOFF) THEN
                        XWATERLEVEL=DMAX1(OCEAN_ELEVATION, &
                    ...
                    SEDX=SEDREMAIN/(DELT)
                    BEDLOAD_FLUX=DMAX1(SEDQMAX,MAXIMUM_SEDIMENT_YIELD(I_NEW,J_NEW))
                    DISCHARGEMAX=DMAX1(DISCHARGEMAX,MAXIMUM_DISCHARGE(I_NEW,J_NEW))
            ...
            KI=I_LOCATION(K-1)
            KJ=J_LOCATION(K-1)
            VOLSTART=STARTING_ELEVATION(K-1)*D2X
            KLOWER=1
            ...
            IF (PROVISIONAL_ELEVATION(K-1) < SEDIMENT_BASE(KI,KJ)) THEN
                ...
                KLOWER = K-1
            ENDIF
            VOLEND=PROVISIONAL_ELEVATION(K-1)*D2X
            IF (K > 2) THEN
                L200: DO  L=K-2,1,-1
                    KI=I_LOCATION(L)
                    KJ=J_LOCATION(L)
                    ...
                    ENDIF
                    IF(KLOWER == 1 ) THEN
                        VOLEND=VOLEND+PROVISIONAL_ELEVATION(L)*D2X
                        VOLSTART=VOLSTART+STARTING_ELEVATION(L)*D2X
                    ENDIF
                ENDDO L200
            ENDIF
    ...
    !        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
     SUBROUTINE SMOOTHSED
        ...
        LOGICAL USEIDO
        REAL (8) :: ENW,ENE,ESE,ESW,EE,EW,EN,ES,EDIFF,ECENTER,WORKING_MATRIXSUM
        USEIDO=.TRUE.
        ...
        L300: DO  IX=I-1,I+1
            ...
            L301: DO JX=J-1,J+1
                ...
                IF (D8_GRADIENT(II,JJ) <= 0.0) THEN
                    FLOW_DIRECTION(II,JJ)=-FLOW_DIRECTION(II,JJ)
    ...
    !        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    !        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    SUBROUTINE PRINT_SEDIMENT_DIAGNOSTICS(I,J,IX,JX,ITYYP,SEDSOURCE)
        USE ERODE_GLOBALS, CHANNEL_STATE_CHANGE=>IDO, SED_SURFACE_ELEVATION=>CFN
        USE SEDROUTE_GLOBALS
        USE SEDDEBUG_GLOBALS
        IMPLICIT NONE
        ...
        ELSE
            IBACK=0
        ENDIF
        WRITE(OUTSEDDEBUG,8119) ITYYP,I,J,K,ITERATION,I_NEW,J_NEW
        8119     FORMAT(           &
        'ITYPE=',I5,' I=',I5,' J=',I5,/,' K=',I5,' ITER=',I5, &
        ' I_NEW=',I5,' J_NEW=',I5)
        ...
        8709           FORMAT(' UPSTREAM_ELEVATION=',G12.5,' END_ELEVATION=',G12.5,  &
        ' EL1=',   &
        G12.5,' EL2=',G12.5,/,' ALLUVIAL_START_GRADIENT=',G12.5,' PROVISIONAL_START_GRADIENT=' &
        ,G12.5, &
        ' PROVISIONAL_END_GRADIENT=',G12.5,' SEDIMENT_VOLUME=',G12.5)
        WRITE(OUTSEDDEBUG,18119) IDEPR,I_LAVA,IBACK
        18119 FORMAT(' IS_DEPRESSION=',I5,' SEDSOURCE=',I5,' DO_BACKTRACK=',I5)
        WRITE(OUTSEDDEBUG,1447)
        1447   FORMAT('  K  I   J   ID CHANNEL_STATE_CHANGE STARTING_ELEVATION      SED_SURFACE_ELEVATION        EL          '&
        ,'     ACTUAL_GRADIENT ', &
        ' ALLUVIAL_GRADIENT','    SEDIMENT_FLUX   ',  &
        '    PROVISIONAL_ELEVATION',    &
        '      NEW_ELEVATION      WATER_LEVEL     DISCHARGE      SEDBASE', &
        '    SEDIMENT_YIELD  EFX ONCE SC  SB  IR  BN')
        IF (K > 0) THEN
            ITEMP1=0
            ITEMP2=0
            ...
            L=0
            IF (COMPLETE_RUNOFF) THEN
                XWATER=DMAX1(OCEAN_ELEVATION,LAKE_SURFACE_ELEVATION(BASIN_NUMBER(IX,JX)))
            ELSE
            ...
            L=K+1
            IF (COMPLETE_RUNOFF) THEN
                XWATER=DMAX1(OCEAN_ELEVATION,LAKE_SURFACE_ELEVATION(BASIN_NUMBER(I_NEW,J_NEW)))
            ELSE
                XWATER=OCEAN_ELEVATION
            ENDIF
