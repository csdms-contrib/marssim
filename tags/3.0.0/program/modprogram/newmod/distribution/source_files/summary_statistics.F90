    ...
    SUBROUTINE CALCULATE_MOMENTS(ARRAY,AVG,VARI,SKEW,KURT,TOTAL)
        ...
        !      *********************************************************************
        IMPLICIT NONE
        REAL (8), INTENT(IN) :: ARRAY(5)
        REAL (8), INTENT(OUT) :: AVG,VARI,SKEW,KURT,TOTAL
        TOTAL=ARRAY(5)
        IF (TOTAL > 0.0) THEN
            AVG=ARRAY(1)/ARRAY(5)
        ELSE
    ...
    SUBROUTINE PRINT_MOMENTS(AVG,VARI,SKEW,KURT,TOTAL)
        ...
        !      *********************************************************************
        REAL (8), INTENT(IN) :: AVG,VARI,SKEW,KURT,TOTAL
    ...
    SUBROUTINE CALCULATE_TOPO_DIVERGENCE()
        ...
        REAL (8) :: TEMP1,  RESULTANT
        REAL (8) :: CMAX,CMIN,CAVG,CSLOPEAVG,CCHANNELAVG
        REAL (8) :: NTOT,CSD
        REAL (8) :: Z1,Z2,Z3,Z4,Z5,Z6,Z7,Z8,Z9,Z10,Z11,Z12
        REAL (8) :: DD,EE,FF,GG,HH,DNM,SLP,DIV
        REAL (8) :: PROFCURV,PLANCURV,PROFPLANSUM,SLOPEDIVSUM
        REAL (8) :: LTBDIVSUM
        REAL (8) :: NSUMMIT,NSINK,NSADDLE,NPLPL,NPLMN,NMNPL,NMNMN
        REAL (8) :: DIVSUMS(5),PLANSUMS(5),PROFSUMS(5),SLOPESUMS(5)
        REAL (8) :: LTBSUMS(5),LTANB
        REAL (8) :: AVG,VARI,SKEW,KURT,TOTAL
        REAL (8) :: SUMXY,SUMX,SUMY,SUMXX,SUMYY,CORREL
        REAL (8) :: ALLTOT,SLPCAT(13),SLPSQCAT(13),DIVCAT(13),DIVSQCAT(13)
        REAL (8) :: PLANCAT(13),PLANSQCAT(13),PROFCAT(13),PROFSQCAT(13)
        REAL (8) :: LTBCAT(13),LTBSQCAT(13),AVGSLPCAT(13)
        REAL (8) :: NUMBCAT(13),LBOUND(13)
        ...
        L100: DO  J=2,MY-1
            M100: DO  I=2,MX-1
                ...
                IF ((D8_GRADIENT(I,J) > 0.0).AND.(DRAINAGE_AREA(I,J) > 0.0)) THEN
                    LTANB=DLOG(DRAINAGE_AREA(I,J)/D8_GRADIENT(I,J))
                ELSE
                    LTANB=0.0
                ENDIF
    ...
    SUBROUTINE PRINT_MORPHOMETRY()
        ...
        REAL (8) :: AM
        !          integer am
        REAL (8) :: EP, EM, EV, AT, AG,MG
        REAL (8) :: RTHICK,RT,RTMIN,RTMAX
        REAL (8) :: STHICK,ST,STMIN,STMAX
    ...
    SUBROUTINE PRINT_SIMULATION_INFORMATION()
        ...
        IMPLICIT NONE
        INTEGER :: I, J, NCC,NS
        REAL (8) :: CA,CMAX,CMIN,SCMAX,SCMIN,SEDIMENT_VOLUME,SEDDIFFERENCE,ELDIFF
        REAL (8) :: DIFFEL,TOTERODE,TOTDEPOSIT,NTOTDEPOSIT,NSEDVOLUME
        REAL (8) ::  SA,SMAX,SMIN,SC,TEMP1,TEMP2,RCA,RCMAX,RCMIN
        REAL (8) :: AVG,VARI,SKEW,KURT,TOTAL
        REAL (8) :: XERODESTATS(5),CRATERSTATS(5),EOSTATS(5),LAVASTATS(5)
        ...
        502 FORMAT( ' ITERATION=',I6,' TIME=',G12.5,' EROSION RATE=',G12.5)
        ...
        IF (ELAPSEDTIME > 0.0) THEN
            DO  I=1,MX
                DO  J=1,MY
                    ...
                    CUMULATIVE_EOLIAN_CHANGE(I,J)=CUMULATIVE_EOLIAN_CHANGE(I,J)/ELAPSEDTIME
                    CUMULATIVE_LAVA_CHANGE(I,J)=CUMULATIVE_LAVA_CHANGE(I,J)/ELAPSEDTIME
                    CUMULATIVE_CRATERING_CHANGE(I,J)=CUMULATIVE_CRATERING_CHANGE(I,J)/ELAPSEDTIME
                    CALL MOMENTS(EOSTATS,CUMULATIVE_EOLIAN_CHANGE(I,J))
                    CALL MOMENTS(LAVASTATS,CUMULATIVE_LAVA_CHANGE(I,J))
    ...
    SUBROUTINE PRINT_VARIABLE_SUMMARIES()
        ...
        REAL (8) :: XX,YY,SUMX,SUMY,SUMXY,TOTAL,SUMXX,SUMYY,CORREL,LTANB
        REAL (8) :: RELIEFRATIOS(6,6,3),REGRSUMS(6,6)
        REAL (8) :: SLOPEANOVA(6,6,3),ESLOPEANOVA(6,6,3),ECHANANOVA(6,6,3)
        DO  I=1,6
            DO  J=1,6
                REGRSUMS(I,J)=0.0
                DO  K=1,3
    ...
    SUBROUTINE CORRELATE(A,B)
        USE ERODE_GLOBALS
        IMPLICIT NONE
        REAL (8), INTENT(IN) :: A(IMMX,JMMX),B(IMMX,JMMX)
        INTEGER :: I,J
        REAL (8) :: NSUM,XSUM,YSUM,XSUMSQ,YSUMSQ,XYSUM,CORR,T1,T2
        NSUM=0.0
        XSUM=0.0
        YSUM=0.0
        XSUMSQ=0.0
    ...
    SUBROUTINE PRINT_BEDROCK_STATISTICS()
        ...
        IMPLICIT NONE
        INTEGER I,J
        REAL (8) :: NSUM,XSUM,YSUM,XSUMSQ,YSUMSQ,XYSUM,CORR
        REAL (8) :: BESUM, BGSUM, BDSUM, BRSUM, BNSUM, RESUM, RGSUM, RDSUM, &
        RRSUM, RNSUM
        !  CALLS: CORRELATE
        WRITE(OUTHIST,100)
    ...
    SUBROUTINE PRINT_RATE_STATISTICS()
        ...
        110   FORMAT(' I=',I5,' EMIN=',G12.5,' EAVG=',G12.5,' EMAX=',G12.5)
        NNNNN=0
        DO  J=1,MY
            DO  I=1,MX
                NNNNN=NNNNN+1
        ...
        1110   FORMAT(' I=',I5,' EMIN=',G12.5,' EAVG=',G12.5,' EMAX=',G12.5)
        NNNNN=0
        DO  J=1,MY
            DO  I=1,MX
                NNNNN=NNNNN+1
        ...
        2110   FORMAT(' I=',I5,' EMIN=',G12.5,' EAVG=',G12.5,' EMAX=',G12.5)
        NNNNN=0
        DO  J=1,MY
            DO  I=1,MX
                IF (.NOT.IS_SEDIMENT_COVERED(I,J)) THEN
        ...
        3110   FORMAT(' I=',I5,' EMIN=',G12.5,' EAVG=',G12.5,' EMAX=',G12.5)
        NNNNN=0
        DO  J=1,MY
            DO  I=1,MX
                IF (.NOT.IS_SEDIMENT_COVERED(I,J)) THEN
